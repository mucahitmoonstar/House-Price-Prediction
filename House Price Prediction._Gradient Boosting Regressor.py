...
# 3. Use a regression model  Gradient Boosting
...
plt.subplot(1, 3, 3)
plt.scatter(y_test, y_pred_gbr, color='orange', edgecolors='k', alpha=0.6)
plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], color='red', linestyle='--')
plt.xlabel('Actual House Prices')
plt.ylabel('Predicted House Prices')
plt.title('Gradient Boosting - Predicted vs Actual House Prices')
plt.grid(True)
...
print(f'Mean Squared Error (MSE): {mean_squared_error(y_test, y_pred_gbr):.2f}')
print(f'R-squared Score: {r2_score(y_test, y_pred_gbr):.2f}')
